/*
  ...
  ____________________
    ...
    [Fbatt] Push the value of battery voltage to floatstack
            This function measures the supply voltage and pushes the value to
            the floatstack. It seems that a standard Arduboy is full charged
            if the voltage exceeds 4.3 V if plugged and 4.2 V if unplugged.
            The battery seems to be empty with a voltage below 3.1 V. These
            values may differ with different hardware or battery condition.
            See also the section "Energy Saving":
  ...
  ____________________
    ...
    FIRST START
      Some variables are stored permanently to the EEPROM and will be loaded
      automatically when started. After flashng AOS the EEPROM may content
      undefined values. So setting some variables after flashing makes
      sense. Particulary this makes sense for the screensaver time ([Sss]),
      screen contrast ([Slit]) and floatstack values ([Fsave]).
    ...
    ENERGY SAVING
      - Some not used features (I2C,ADC,USART, all timers but number 0) are
        permanently disabled.
      - The display deactivates after the screensaver time (set with [Sss]).
      - After 3 minutes of inactivity auto power off activates the deep sleep mode.
      Note: In deep sleep mode USB and all timers (clock) are disabled.
      Note: While the clock is active ([WATCH]) neither aoto poweroff nor the
            screensaver wil be activated.
  ...
  ____________________
    ...
    LEONARDO's DEFINITIONS FOR MODIFIEER KEYS (Key Hex Dec)
...
const byte font5x8 [] PROGMEM = {
  ...
  0x7f, 0x41, 0x49, 0x41, 0x7f, // 0 high digital
  0x00, 0x00, 0x7f, 0x00, 0x00, // 1 high digital
  ...
  0x07, 0x7f, 0x01, 0x7f, 0x01  // ~ paragraph
};
...
static void Pexp(void) {
  // Open shell on Linux PC (ALT-F1 on Debian systems)
  Keyboard.press(KEY_LEFT_ALT); Keyboard.press(KEY_F1); delayshort(200); Keyboard.releaseAll();
  // Open program minicom on Linux PC, send program and save it as prg.aos
  ...
  delayshort(50);
  Keyboard.print(F("exit")); Keyboard.write(KEY_RETURN); // Exit shell
  oldmode = CMDMODE;
  mode = MSGMODE;
  msg = MSGOK;
}
